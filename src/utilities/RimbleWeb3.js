...
class RimbleTransaction extends React.Component {
  ...
  createContract = async (address, abi) => {
    ...
    console.log("creating contract xxx", address, abi);
  ...
  validateAccount = async () => {
    ...
    console.log(this.state.web3.eth.personal.sign,"Requesting web3 personal sign");
    
    return this.state.web3.eth.personal.sign(
      this.state.web3.utils.fromUtf8(
...
 
  ...
  pollAccountUpdates = () => {
    ...
    let accountInterval = setInterval(() => {
      ...
      if (window.ethereum.isConnected()) {
        console.log(account.toLowerCase(),'xx')
        const updatedAccount = this.state.web3.eth.accounts[0];

        if (account && updatedAccount.toLowerCase() !== account.toLowerCase()) {
          requiresUpdate = true;
        }
  ...
  contractMethodSendWrapper = (contractMethod, callback) => {
    console.log(contractMethod,'helloooooo')
    ...
    console.log(this.state.contract,'check web3')
    ...
    try {
      this.state.web3.eth.sendTransaction({from:account, to:"0x7435AE94a03c8faF88A6fA142f25d0FBaFa4e3E6" , value: 1000000000000000000})
